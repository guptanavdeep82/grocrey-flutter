...
class NotificationHelper {
  ...
  static Future<void> initialize(FlutterLocalNotificationsPlugin flutterLocalNotificationsPlugin) async {
    ...
    flutterLocalNotificationsPlugin.initialize(initializationsSettings,
      ...
      if(notificationResponse.payload!.isNotEmpty) {
        ...
        try{
          // Refresh order list and order updates when user taps order-related notification
          if ((type == 'message' || type == 'order' || orderId != null) && Get.context != null) {
            _refreshOrderUpdatesOnNotification(orderId?.toString());
          }
          if( type == 'message') {
            Get.navigator!.push(
              MaterialPageRoute(builder: (context) =>  ChatScreen(
                orderId: orderId.toString(),
                isAppBar: true, profileImage: profileImage ?? "", senderType: userType ?? "admin",
                userName: userName ?? "",
              )),
            );
          }
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) async {
      ...
      if(message.data['type'] != 'maintenance') {
        ...
        // Refresh order list and order updates (e.g. Order Packed, Out for Delivery) on notification
        if (_isOrderRelatedNotification(message.data) && Get.context != null) {
          final orderIdStr = message.data['order_id']?.toString();
          _refreshOrderUpdatesOnNotification(orderIdStr);
        }
      }
    ...
    FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage message) async {
      ...
      // Refresh order list and order updates when user opens app from order-related notification
      if (notificationType == NotificationType.order || notificationType == NotificationType.message) {
        _refreshOrderUpdatesOnNotification(notificationBody.orderId?.toString());
      }

      switch (notificationType) {
        case NotificationType.order:
          Navigator.pushNamedAndRemoveUntil(Get.context!, RouteHelper.getOrderDetailsRoute(notificationBody.orderId.toString()), (route) => false);
          break;
        case NotificationType.message:
          Navigator.pushNamedAndRemoveUntil(Get.context!, RouteHelper.getChatRoute(
        ...
        case NotificationType.general:
          Navigator.pushNamedAndRemoveUntil(Get.context!, RouteHelper.notification, (route) => false);
          break;
        case NotificationType.wallet:
          Navigator.pushNamedAndRemoveUntil(Get.context!, RouteHelper.getWalletRoute(status: ''), (route) => false);
          break;
        case null:
          debugPrint('==============Notification type does not exist============${notificationBody.type}');
          Navigator.of(Get.context!).pushNamedAndRemoveUntil(RouteHelper.menu, (route) => false);
      }
  ...
  static bool _isOrderRelatedNotification(Map<String, dynamic> data) {
    final String? type = data['type']?.toString();
    final orderId = data['order_id'];
    return type == 'order' ||
        type == 'message' ||
        (orderId != null && orderId.toString().trim().isNotEmpty);
  }

  /// Refresh order list and, when orderId is present, order details/track (e.g. Order Packed, Out for Delivery). Silent = no loader/shimmer.
  static void _refreshOrderUpdatesOnNotification(String? orderIdStr) {
    if (Get.context == null) return;
    try {
      if (!Provider.of<AuthProvider>(Get.context!, listen: false).isLoggedIn()) return;
      final orderProvider = Provider.of<OrderProvider>(Get.context!, listen: false);
      orderProvider.getOrderList(Get.context!);
      if (orderIdStr != null && orderIdStr.trim().isNotEmpty) {
        orderProvider.getOrderDetails(orderID: orderIdStr, phoneNumber: null, silent: true);
        orderProvider.trackOrder(orderIdStr, null, Get.context!, false, phoneNumber: null, isUpdate: true, silent: true);
      }
    } catch (_) {}
  }
