...
class CheckOutHelper {
  ...
  static List<PaymentMethod> getActivePaymentList({required ConfigModel configModel}){
    ...
    paymentMethodList.addAll(getProcessedPaymentList(configModel: configModel));

    return paymentMethodList;
  }

  static List<PaymentMethod> getProcessedPaymentList({required ConfigModel configModel}) {
    List<PaymentMethod> paymentMethodList = [];
    ...
    for (var method in (configModel.activePaymentMethodList ?? [])) {
      if (method.getWay!.toLowerCase().contains('razor') || method.getWay!.toLowerCase().contains('rezo')) {
        paymentMethodList.add(PaymentMethod(
          getWay: method.getWay,
          getWayTitle: 'UPI Pay',
          getWayImage: Images.upi,
          type: method.getWay,
        ));
        paymentMethodList.add(PaymentMethod(
          getWay: method.getWay,
          getWayTitle: 'Credit Card',
          getWayImage: Images.creditCard,
          type: method.getWay,
        ));
        paymentMethodList.add(PaymentMethod(
          getWay: method.getWay,
          getWayTitle: 'Net Banking',
          getWayImage: Images.netBank,
          type: method.getWay,
        ));
        paymentMethodList.add(PaymentMethod(
          getWay: method.getWay,
          getWayTitle: 'PhonePe',
          getWayImage: Images.phonepe,
          type: method.getWay,
        ));
        paymentMethodList.add(PaymentMethod(
          getWay: method.getWay,
          getWayTitle: 'Google Pay',
          getWayImage: Images.googlePay,
          type: method.getWay,
        ));
  ...
  static Future<void> selectDeliveryAddressAuto({AddressModel? lastAddress, required bool isLoggedIn, required String? orderType}) async {
  ...
  static double getMinimumDeliveryChargeForDistanceWise(){
    ...
    return splashProvider.deliveryInfoModelList?[orderProvider.branchIndex].deliveryChargeSetup?.minimumDeliveryCharge?.toDouble() ?? 0.0;
  }
