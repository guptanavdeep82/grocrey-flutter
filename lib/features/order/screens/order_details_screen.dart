...
class _OrderDetailsScreenState extends State<OrderDetailsScreen> {
  ...
  void _refreshOrderUpdates(BuildContext context) {
    final orderProvider = Provider.of<OrderProvider>(context, listen: false);
    final String? orderId = widget.orderId?.toString() ?? orderProvider.trackModel?.id?.toString();
    if (orderId == null) return;
    orderProvider.getOrderDetails(orderID: orderId, phoneNumber: widget.phoneNumber, silent: true);
    orderProvider.trackOrder(orderId, null, context, false, phoneNumber: widget.phoneNumber, isUpdate: true, silent: true);
  }
  ...
  void initState() {
    ...
    // Auto order updates (e.g. Order Packed, Out for Delivery) - 30 second API call
    _refreshTimer = Timer.periodic(const Duration(seconds: 30), (_) {
      if (!mounted) return;
      final route = ModalRoute.of(context);
      if (route?.isCurrent != true) return;
      _refreshOrderUpdates(context);
    });
  }
