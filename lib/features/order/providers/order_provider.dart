...
import 'package:flutter_grocery/features/payment/services/razorpay_service.dart';
...
class OrderProvider extends ChangeNotifier {
  ...
  final ScrollController timeSlotScrollController = ScrollController();
  ...
  void updateTimeSlot(int index) {
    ...
    if (timeSlotScrollController.hasClients) {
      timeSlotScrollController.animateTo(
        index * itemWidth,
  ...
  Future<List<OrderDetailsModel>?> getOrderDetails({required String orderID, String? phoneNumber, bool silent = false}) async {
    if (!silent) {
      _orderDetails = null;
      _isLoading = true;
  ...
  Future<ResponseModel> trackOrder(
    ...
    if (!silent) {
      _trackModel = null;
      if (!fromTracking) _orderDetails = null;
    }
    _showCancelled = false;
    if (orderModel == null) {
      if (!silent) {
  ...
  Future<bool> getDistanceInMeter(LatLng originLatLng, LatLng destinationLatLng) async {
    ...
    try {
      if (response.response!.statusCode == 200 && response.response!.data[0]['distanceMeters'] != null) {
        ...
        _distance =
            Geolocator.distanceBetween(originLatLng.latitude, originLatLng.longitude, destinationLatLng.latitude, destinationLatLng.longitude) / 1000;
      }
    } catch (e) {
      _distance =
          Geolocator.distanceBetween(originLatLng.latitude, originLatLng.longitude, destinationLatLng.latitude, destinationLatLng.longitude) / 1000;
    ...
    if (Provider.of<AuthProvider>(Get.context!, listen: false).isLoggedIn()) {
      Provider.of<ProfileProvider>(Get.context!, listen: false).getUserInfo(true, isUpdate: true).then((value) async {
        ...
        if (Provider.of<AuthProvider>(Get.context!, listen: false).isLoggedIn() &&
          ...
          await ResponsiveHelper()
              .showDialogOrBottomSheet(
                Get.context!,
                IncompleteOfflinePaymentDialog(order: userData.lastIncompleteOfflineBooking),
  ...
  Future<void> switchPaymentMethod({required String orderId, required String paymentMethod, int isPartial = 0, double? bringChangeAmount}) async {
    _isLoading = true;
    notifyListeners();
    ApiResponseModel apiResponse = await orderRepo!.switchPaymentMethod(
      orderId: orderId,
  ...
  Future<String?> placeDigitalOrder(BuildContext context, PlaceOrderModel placeOrderBody) async {
    ...
    if (apiResponse.response?.statusCode == 200 && apiResponse.response.data['redirect_link'] != null) {
  ...
  Future<Map<String, dynamic>?> createRazorpayOrder({required double amount, required String currency, required String paymentRequestId}) async {
    _isLoading = true;
    notifyListeners();
    ApiResponseModel apiResponse = await orderRepo!.createRazorpayOrder(amount: amount, currency: currency, paymentRequestId: paymentRequestId);
    _isLoading = false;
    notifyListeners();
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
  ...
  Future<bool> verifyRazorpayPayment(Map<String, dynamic> data) async {
    _isLoading = true;
    notifyListeners();
    ApiResponseModel apiResponse = await orderRepo!.verifyRazorpayPayment(data);
    _isLoading = false;
    notifyListeners();
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      return true;
  ...
  void initRazorpay() {
    _razorpayService ??= RazorpayService(onSuccess: _handleRazorpaySuccess, onFailure: _handleRazorpayFailure);
  }

  void disposeRazorpay() {
    _razorpayService?.dispose();
    _razorpayService = null;
  }

  PlaceOrderModel? _pendingPlaceOrderBody;

  Function? _razorpayCallback;

  void _handleRazorpaySuccess(String paymentId, String orderId, String signature) async {
    if (_pendingPlaceOrderBody == null) return;

    bool isVerified = await verifyRazorpayPayment({
      'payment_id': paymentId,
      'order_id': orderId,
      'signature': signature,
      'payment_request_id': _paymentRequestId,
    });

    if (isVerified) {
      if (_pendingPlaceOrderBody!.shwitchToDigital == '1') {
        // Switch flow
        await trackOrder(_pendingPlaceOrderBody!.orderId, null, Get.context!, false, isUpdate: true);
        await getOrderDetails(orderID: _pendingPlaceOrderBody!.orderId!);
        _razorpayCallback?.call(true, 'payment_verified_successfully', _pendingPlaceOrderBody!.orderId);
      } else {
        // New order flow
        await placeOrder(_pendingPlaceOrderBody!, (bool isSuccess, String message, String orderID) {
          _razorpayCallback?.call(isSuccess, message, orderID);
        });
      }
    } else {
      _razorpayCallback?.call(false, 'payment_verification_failed', '-1');
    }
  ...
  void _handleRazorpayFailure(String message) {
    _razorpayCallback?.call(false, message, '-1');
    _pendingPlaceOrderBody = null;
  }

  Future<void> startRazorpayPayment({
    required PlaceOrderModel placeOrderBody,
    required Function callback,
    ...
    _razorpayCallback = callback;

    _isLoading = true;
    notifyListeners();

    ApiResponseModel digitalResponse = await orderRepo!.paymentMobile(placeOrderModel: placeOrderBody);

    if (digitalResponse.response?.statusCode == 200 && digitalResponse.response.data['redirect_link'] != null) {
      String redirectLink = digitalResponse.response.data['redirect_link'];
      _paymentRequestId = Uri.parse(redirectLink).queryParameters['payment_id'];

      if (_paymentRequestId != null) {
        final response = await createRazorpayOrder(amount: placeOrderBody.orderAmount!, currency: 'INR', paymentRequestId: _paymentRequestId!);

        if (response != null && response['order_id'] != null) {
          String rzpOrderId = response['order_id'];
          String key = (response['key'] ?? response['razor_key'] ?? '').toString().trim();
          if (key.isEmpty && Get.context != null) {
            key = (Provider.of<SplashProvider>(Get.context!, listen: false).configModel?.razorPayKey ?? '').toString().trim();
          }
          if (key.isEmpty) {
            _isLoading = false;
            notifyListeners();
            showCustomSnackBarHelper('Unable to start Razorpay. Payment key is missing.');
            return;
          }
          int amount = (response['amount'] is int ? response['amount'] : (double.parse(response['amount'].toString())).toInt());
          ...
          _razorpayService?.startPayment(
            ...
            email: email,
            name: name,
            description: 'Order from ${AppConstants.appName}',
            orderId: rzpOrderId,
        ...
        _isLoading = false;
        notifyListeners();
        showCustomSnackBarHelper('Failed to get payment request id');
      }
    } else {
      _isLoading = false;
      notifyListeners();
      ApiCheckerHelper.checkApi(digitalResponse);
    }
  }
}
