...
class _AddFundDialogueWidgetState extends State<AddFundDialogueWidget> {
  ...
  void initState() {
    ...
    if(configModel != null) {
      _paymentList = CheckOutHelper.getProcessedPaymentList(configModel: configModel);
    }
            ...
            return Consumer<OrderProvider>(builder: (context, orderProvider, _) {
              return Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  if(inputAmountController.text.isNotEmpty) PaymentMethodView(
                    onTap: (method) => orderProvider.changePaymentMethod(
                  ...
                  CustomButtonWidget(
                    ...
                    onPressed: () async {
                      ...
                      if(inputAmountController.text.isEmpty){
                        ...
                        final double amount = double.parse(inputAmountController.text.replaceAll(configModel!.currencySymbol!, ''));

                        if(amount == 0) {
                          showCustomSnackBarHelper(getTranslated('amount_must_be_greater_than_zero', context));
                          return;
                        }
                        ...
                        final paymentUrl = await walletProvider.getAddFundWalletUrl(context,
                          customerId: profileProvider.userInfoModel?.id,
                          paymentMethod: orderProvider.paymentMethod?.getWay,
                          paymentPlatform: kIsWeb ? 'web' : 'app',
                          amount: amount,
                          callback: kIsWeb
                              ? '$protocol//$hostname${kDebugMode ? ':$port' : ''}${'${RouteHelper.wallet}/add_fund'}'
                              : '${AppConstants.baseUrl}${RouteHelper.wallet}',
                        ...
                        if(paymentUrl != null){
                          if(kIsWeb){
                            ...
                            if(context.mounted) Navigator.pushReplacementNamed(context, RouteHelper.getPaymentRoute(url: paymentUrl));
                          }
                        }
                      }
                    },
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start ,mainAxisSize: MainAxisSize.min, children: [
        ...
        ListView.builder(
          ...
          itemBuilder: (context, index){
            ...
            bool isSelected = finalPaymentList[index].getWayTitle == selectedPaymentMethod;
            bool isOffline = finalPaymentList[index].type == 'offline';

            return Opacity(
              opacity: (isOffline && hideOffline) || (!isOffline && hideDigital) ? 0.4 : 1,
              child: InkWell(
                onTap: (isSelected && isOffline) || (isOffline && hideOffline) || (!isOffline && hideDigital) ? null : ()=> onTap(finalPaymentList[index]),
                child: Container(
                  decoration: BoxDecoration(
