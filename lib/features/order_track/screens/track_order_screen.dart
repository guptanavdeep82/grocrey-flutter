...
import 'package:flutter_grocery/features/order_track/widgets/track_map_button_widget.dart';
...
class _TrackOrderScreenState extends State<TrackOrderScreen> {
  ...
  void initState() {
    ...
    // Auto order updates (e.g. Order Packed, Out for Delivery) - 30 second API call (silent, no loader/shimmer)
    _refreshTimer = Timer.periodic(const Duration(seconds: 30), (_) {
      if (!mounted) return;
      final route = ModalRoute.of(context);
      if (route?.isCurrent != true) return;
      if (widget.orderID == null) return;
      orderProvider.trackOrder(widget.orderID, widget.orderModel, context, true, isUpdate: true, phoneNumber: widget.phone, silent: true).whenComplete(() {
        if (mounted && orderProvider.trackModel?.deliveryMan != null) {
          orderProvider.getDeliveryManData(context, orderId: widget.orderID, deliverymanId: orderProvider.trackModel?.deliveryMan?.id.toString() ?? "");
        }
      });
    });
