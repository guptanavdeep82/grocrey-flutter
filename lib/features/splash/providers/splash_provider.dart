...
class SplashProvider extends ChangeNotifier {
  ...
  Future<ConfigModel?> initConfig({bool fromNotification = false, DataSourceEnum source = DataSourceEnum.local}) async {
    if (source == DataSourceEnum.local) {
      ...
      ApiResponseModel<dio_pkg.Response> apiResponseModel = await splashRepo!.getConfig(source: DataSourceEnum.client);
  ...
  Future<void> _onConfigAction(bool fromNotification) async {
    if (_configModel != null) {
      ...
      if (fromNotification) {
        if (MaintenanceHelper.isMaintenanceModeEnable(configModel) &&
            (MaintenanceHelper.checkCustomerMaintenanceMode(configModel) || MaintenanceHelper.checkWebMaintenanceMode(configModel))) {
  ...
  bool getAcceptCookiesStatus(String? data) =>
      splashRepo!.sharedPreferences!.getString(AppConstants.cookingManagement) != null &&
  ...
  void initializeScreenList() {
    _screenList = [
      ...
      MainScreenModel(const OrderSearchScreen(), 'order_track', Images.orderDetails),
      ...
      if (_configModel?.walletStatus ?? false) MainScreenModel(const WalletScreen(), 'wallet', Images.wallet),
      if (_configModel?.loyaltyPointStatus ?? false) MainScreenModel(const LoyaltyScreen(), 'loyalty_point', Images.loyaltyIcon),
      ...
      if (_configModel?.refundPolicyStatus ?? false)
        MainScreenModel(const HtmlViewerScreen(htmlType: HtmlType.refundPolicy), 'refund_policy', Images.referralIcon),
