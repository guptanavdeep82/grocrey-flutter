...
class _SplashScreenState extends State<SplashScreen> {
  ...
  void _route() {
    ...
    splashProvider.initConfig(source: DataSourceEnum.local).then((configModel) async {
      _onConfigAction(configModel, splashProvider, Get.context!);
    });
  }

  void _onConfigAction(ConfigModel? configModel, SplashProvider splashProvider, BuildContext context) {
    if (configModel != null) {
      ...
      Future.delayed(const Duration(milliseconds: 5)).then((_) {
        if (AppConstants.appVersion < minimumVersion && !ResponsiveHelper.isWeb()) {
          Navigator.pushNamedAndRemoveUntil(Get.context!, RouteHelper.getUpdateRoute(), (route) => false);
        } else {
          if (MaintenanceHelper.isMaintenanceModeEnable(configModel) && MaintenanceHelper.isCustomerMaintenanceEnable(configModel)) {
            ...
            if (Provider.of<SplashProvider>(Get.context!, listen: false).showIntro()) {
              Navigator.pushNamedAndRemoveUntil(Get.context!, RouteHelper.onBoarding, (route) => false, arguments: OnBoardingScreen());
            } else {
              Navigator.of(Get.context!).pushNamedAndRemoveUntil(RouteHelper.menu, (route) => false);
            }
          }
  ...
  void _checkConnectivity() {
    ...
    subscription = Connectivity().onConnectivityChanged.listen((List<ConnectivityResult> result) {
      ...
      if ((isFirst && !isConnected) || !isFirst && context.mounted) {
        showCustomSnackBarHelper(getTranslated(isConnected ? 'connected' : 'no_internet_connection', Get.context!), isError: !isConnected);
      ...
      body: Consumer<SplashProvider>(
        builder: (context, splashProvider, _) {
          ...
          return Column(
            ...
            children: [
              Image.asset(Images.logo, height: 130, width: 500),
              const SizedBox(height: Dimensions.paddingSizeSmall),

              Text(
                AppConstants.appName,
                textAlign: TextAlign.center,
                style: poppinsMedium.copyWith(color: Theme.of(context).primaryColor, fontSize: 30),
              ),
            ],
          );
        },
