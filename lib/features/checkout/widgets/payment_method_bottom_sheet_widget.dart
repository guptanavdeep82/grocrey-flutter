...
class _PaymentMethodBottomSheetWidgetState extends State<PaymentMethodBottomSheetWidget> {
  ...
  void initState() {
    ...
    paymentList.addAll(CheckOutHelper.getProcessedPaymentList(configModel: configModel));
  ...
  Future<void> _switchOfflineToDigital({required String paymentMethod, required double orderAmount, double? partialAmount, required  String orderId}) async {
    ...
    if ((paymentMethod == 'razorpay' || paymentMethod == 'razor_pay') && !kIsWeb) {
      orderProvider.initRazorpay();
      orderProvider.startRazorpayPayment(
        placeOrderBody: placeOrderBody,
        callback: (bool isSuccess, String message, String orderID) {},
      ...
      placeOrderBody = placeOrderBody.copyWith(
