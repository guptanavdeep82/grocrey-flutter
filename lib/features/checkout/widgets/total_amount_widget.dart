...
class TotalAmountWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Text(
          '${getTranslated('tax', context)} ${(Provider.of<SplashProvider>(context, listen: false).configModel?.isVatTexInclude ?? false) ? '(${getTranslated('include', context)})' : ''}',
          style: poppinsRegular.copyWith(
            color: Theme.of(context).hintColor.withValues(alpha: 0.5),
            fontSize: Dimensions.fontSizeDefault,
          ),
        ...
        CustomDirectionalityWidget(child: Text(
          '${(Provider.of<SplashProvider>(context, listen: false).configModel?.isVatTexInclude ?? false) ? '' : '+'} ${PriceConverterHelper.convertPrice(context, tax ?? 0)}',
          style: poppinsMedium.copyWith(fontSize: Dimensions.fontSizeDefault),
        )),
      ]),
      const SizedBox(height: Dimensions.paddingSizeSmall),
