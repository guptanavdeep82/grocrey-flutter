  ...
  void route(
    ...
    if (isRoute) {
      if (token != null) {
        ...
        Navigator.pushNamed(context, RouteHelper.getOtpRegistration(tempToken, email ?? '', userName: name ?? ''));
        ...
        if (authProvider.countSocialLoginOptions == 1) {
          return Row(
            children: [
              if (AuthHelper.isGoogleLoginEnable(configModel))
                Expanded(
                  child: InkWell(
                    ...
                    child: SocialLoginButtonWidget(text: getTranslated('continue_with_google', context), image: Images.google),
                  ),
                ),
              ...
              //     onTap: () async{
              //       final SocialLoginModel? socialLoginModel = await authProvider.facebookLogin();
              ...
              if (AuthHelper.isGoogleLoginEnable(configModel)) ...[
                Expanded(
                  child: InkWell(
                    ...
                    child: SocialLoginButtonWidget(text: getTranslated('google', context), image: Images.google),
                  ),
                ),
            ...
            children: [
              if (AuthHelper.isGoogleLoginEnable(configModel)) ...[
                InkWell(
                  ...
                  child: const SocialLoginButtonWidget(image: Images.google, padding: EdgeInsets.all(Dimensions.paddingSizeSmall)),
                ),
...
class SocialLoginButtonWidget extends StatelessWidget {
  ...
  const SocialLoginButtonWidget({super.key, this.text, required this.image, this.color, this.padding});
      ...
      child: Row(
        ...
        children: [
          ...
          if (text != null) ...[
            ...
            Text(
              text!,
              style: poppinsSemiBold.copyWith(fontSize: Dimensions.fontSizeDefault, color: Theme.of(context).textTheme.bodyMedium?.color),
            ),
          ],
        ],
      ),
