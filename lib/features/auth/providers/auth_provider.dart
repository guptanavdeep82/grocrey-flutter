...
class AuthProvider with ChangeNotifier {
    ...
    if (kIsWeb) {
      ...
      try {
        if (signIn.supportsAuthenticate()) {
          await signIn.initialize(serverClientId: AppConstants.googleServerClientId).then((_) async {
  ...
  // Future<SocialLoginModel?> facebookLogin() async {
      ...
      if (isForgetPassword) {
        ...
        if (token != null) {
          ...
          saveUserNumberAndPassword(
            UserLogData(countryCode: countryCode, phoneNumber: phone, email: null, password: null, loginType: FromPage.otp.name),
          );
