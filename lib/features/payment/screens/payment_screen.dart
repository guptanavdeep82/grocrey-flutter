...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  Widget build(BuildContext context) {
    return PopScope(
      canPop: false,
      onPopInvokedWithResult: ((_, _) => _exitApp(context).then((value) => value!)),
      child: Scaffold(
        body: Center(
          child: Stack(
            children: [
...
class MyInAppBrowser extends InAppBrowser {
  ...
  void _pageRedirect(String url) {
    if(_canRedirect) {
      bool checkedUrl = (url.contains('${AppConstants.baseUrl}${RouteHelper.orderSuccessful}') || url.contains('${AppConstants.baseUrl}${RouteHelper.wallet}') ||  url.contains('${AppConstants.baseUrl}${RouteHelper.orderDetails}'));
      ...
      bool isOrderDetails = url.contains('${AppConstants.baseUrl}${RouteHelper.orderDetails}');
      ...
      if(isSuccess){
        ...
        if(isWallet){
          ...
          Navigator.pushReplacementNamed(context, RouteHelper.getOrderDetailsRoute(orderId.toString()));
        }else{
          if(token.isNotEmpty) {
